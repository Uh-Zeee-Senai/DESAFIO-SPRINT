...
window.addEventListener("DOMContentLoaded", () => {
    ...
    // 🔑 Verifica se os IDs do HTML estão sendo capturados corretamente
    menuDiv = document.getElementById("menu"); 
    gameDiv = document.getElementById("game"); // Era o antigo #gameCanvasContainer no seu código original
    ...
    // 🔑 Event listeners: Se o botão não funcionar, o problema está aqui ou no HTML
    if (startBtn) startBtn.addEventListener("click", onStart);
    if (resetDataBtn) resetDataBtn.addEventListener("click", ()=>{ localStorage.removeItem("lastPlayer"); nameInput.value=""; debug("Saved name cleared"); });
...
function onResize() {
    ...
    if (hudMinimapCanvas) {
        // Redimensiona o canvas do minimapa baseado no tamanho do seu pai (CSS)
        if (hudMinimapCanvas.parentElement) {
...
function onStart() {
    ...
    // Esconde o menu e mostra o HUD (agora que o #game é o HUD container)
...
function initRace(playerName) {
    ...
    player = {
        ...
        img: IMG.player,
        x: W/2 - 55,
        y: H * CONFIG.CAR_BASE_Y_PERC, // USANDO PORCENTAGEM (75%)
...
function render() {
    ...
    ctx.fillStyle = "red";
    ctx.beginPath();
    // Ponto central do carro: (x + width/2, y + height/2)
    ctx.arc(player.x + player.width / 2, player.y + player.height / 2, 10, 0, Math.PI * 2); 
    ctx.fill();
    ctx.fillStyle = "white";
    ctx.font = "16px Arial";
    ctx.fillText("Player Y: " + player.y.toFixed(0), player.x, player.y - 10);
    // FIM DO DEBUG VISUAL
...
function drawCar(c, fixedY) {
    ...
    if (img && img.complete && img.naturalWidth) {
        ...
        // FALLBACK DE DEBUG SIMPLIFICADO: Não usa transformações
        ...
        // Desenha na coordenada x e y, sem rotação
    ...
    easterTimer = setTimeout(() => {
        if (gameRunning) {
            easter = {
                type: 'easter',
                img: IMG.easter,
                x: (Math.random() > 0.5 ? -1 : 1) * 0.2, // Posição lateral (esquerda ou direita)
                z: player.totalDistance + totalTrackLength * 0.2, // 20% da pista à frente
                width: 60, height: 60
            };
